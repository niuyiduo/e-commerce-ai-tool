...
🌐 **🌐 在线体验地址**：https://e-commerce-ai-tool.vercel.app

> ⚠️ **重要提示**：
> 
> 线上部署受 **Vercel 免费版限制**（10 秒 Serverless 函数执行时间），可能导致：
> - ❌ AI 分析超时（尤其是 Thinking 模型）
> - ❌ 请求被强制中断
> - ❌ 生成失败
> 
> 💡 **强烈推荐使用本地环境**获得最佳体验！本地运行无任何限制，响应快速稳定。

> ⚠️ **注意**：线上部署受 Vercel 免费版限制（10 秒执行时间），AI 分析可能超时。**强烈推荐使用本地环境**获得最佳体验！  
...
- **智能尺寸标准化** 🆕：
  - 横版图片：固定宽度 1200px，高度按比例缩放
  - 竖版图片：固定高度 1200px，宽度按比例缩放
  - 确保AI生成内容在各类设备清晰展示
- **对话数据隔离机制** 🆕：
  - 重新上传图片时自动清空历史对话
  - 避免上下文混淆，保障数据独立性
...
- 🖼️ **边框风格选择**（仅 Thinking 模型，纯本地Canvas绘制）：
  - **简约边框**：白灰双线，简洁大方 + **缩放动画**（1.0-1.03倍周期性）
  - **国潮边框**：红金渐变 + 四角菱形装饰 + **从上到下流动**（透明遮罩层）
  - **渐变边框**：紫粉蓝渐变 + 发光阴影效果 + **从中心扩散**（彩虹渐变）
  - **豪华边框**：三层金色 + 四角宝石装饰 + **从左到右流动**（金色光带）
  - 边框宽度 25px，明显视觉区分
- 🔄 **智能反馈优化**：
...
- **智能尺寸适配** 🆕：
  - 横版图片或混合：1280x720（16:9）横版视频
  - 全竖版图片：720x1280（9:16）竖版视频
  - 混合上传时清晰提示用户
  - 便于虚拟人在右上角展示
- **视频参数配置**：
  - 时长设置：3-10 秒
...
**⭐ 顶级 VRoid 形象**（最新推荐）：
- 🎨 VRoid Studio专业3D虚拟人物（基于 Three.js 渲染）
- 👫 **男女双性别支持**：根据配音声音自动匹配形象
- 💫 **真实表情驱动**：52种ARKit标准表情系统
- 🗣️ **精确口型同步**：15种口型（aa/ih/ou/ee等）
- ⏱️ **音频时长精准控制**：配音停止时人物立即停止动作
- 🎭 **静止姿态优化**：
  - 配音时：仅表情和口型变化，身体静止不旋转
  - 配音结束：自动恢复默认微笑表情
  - 手臂自然垂放身体两侧，正面朝向摄像机
- 📹 **脸部特写模式**：聚焦面部表情，清晰展示口型与眨眼
- 🎯 **超高清渲染**：1200×1200分辨率 + 高质量图像插值
...
- **图像处理**：Canvas API（包含本地边框绘制）
- **视频生成**：MediaRecorder API
- **3D 渲染** ：Three.js + @pixiv/three-vrm（VRM虚拟形象）
  - 顶级VRoid：1200×1200超高清渲染 + 脸部特写
  - 高级Q版：1200×1200超高清渲染 + 大半上身
  - 独立摄像机配置，互不影响
  - 高质量图像平滑插值，消除缩放模糊
- **音频处理**：Web Audio API + 火山引擎 TTS
- **本地存储** ：MySQL 持久化存储（所有装饰图记录）
...
| 对比项目 | 本地运行 | Vercel 线上部署 |
|---------|---------|----------------|
| 超时限制 | ✅ 无限制（Thinking 90s, Vision 30s） | ⚠️ 10秒强制中断 |
| 响应速度 | ⚡ 极快（无网络延迟） | 🐌 较慢（冷启动+网络） |
| 功能完整性 | 🎯 所有功能正常 | ⚠️ AI分析易超时 |
| 调试便利性 | 🐛 实时日志+热更新 | ❌ 无法调试 |
| 适用场景 | 💻 开发、测试、演示 | 🌐 快速分享 |

💡 **建议**：开发和测试时使用本地环境，需要分享时使用线上地址
...
确保 `.env.local` 文件中已配置火山引擎 API Key：

```env
VOLCENGINE_API_KEY=你的火山引擎 API 密钥
...
1. **⚡ 无超时限制**
   - Thinking 模型：90 秒完整推理时间
   - Vision 模型：30 秒分析时间
   - 完全由代码控制，不受 Vercel 10秒限制
...
│   │   ├── canvas/
│   │   │   └── imageGenerator.ts # 图片生成与装饰（含本地边框）
│   │   ├── db.ts                 # MySQL 数据库连接 🆕
│   │   ├── tts/
...
| | | - 确定火山引擎 AI 服务 | |
| **Week 1** | 第1周 | 项目初始化与基础搭建 | 项目框架 |
| | | - 初始化 Next.js 项目 | |
| | | - 配置 Tailwind CSS | |
| | | - 搭建基础 UI 框架 | |
| | | - 实现图片上传功能（点击+拖拽） | |
| **Week 2** | 第2周 | AI 对话功能开发 | AI 对话模块 |
| | | - 集成火山引擎 API | |
| | | - 实现 /api/chat 路由 | |
| | | - 开发对话界面 | |
| | | - 实现商品文案生成 | |
| **Week 2** | 第2周 | 图片装饰功能开发 | 图片处理模块 |
| | | - 开发 Canvas 图片处理 | |
| | | - 实现普通装饰模式（徽章+贴纸） | |
| | | - 添加水印功能 | |
| | | - 实现图片下载 | |
| **Week 2** | 第2周 | 视频生成功能开发 | 视频生成模块 |
| | | - 实现多图上传管理 | |
| | | - 开发 MediaRecorder 视频录制 | |
| | | - 实现转场效果 | |
| | | - 添加字幕功能 | |
| **Week 3** | 第3周 | 语音配音功能开发 | TTS 语音模块 |
| | | - 集成火山引擎 TTS API | |
| | | - 实现 /api/tts 路由 | |
| | | - 开发音视频同步逻辑 | |
| | | - 实现 Web Audio API 混音 | |
| **Week 3** | 第3周 | 多模型切换与高级功能 | 高级功能模块 |
| | | - 实现 6 个 AI 模型切换 | |
| | | - 开发高级定制装饰图（Vision） | |
| | | - 实现智能反馈优化机制 | |
| | | - 添加边框风格选择 | |
| **Week 1-3** | 第1-3周 | 部署上线与优化 | 生产环境 |
| | | - GitHub 仓库配置 | |
| | | - Vercel 部署配置 | |
| | | - CI/CD 流程搭建 | |
| | | - 性能优化与文档编写 | |
...
- `generateSmartDecorativeImage()` - 高级定制装饰图生成
- `drawBorder()` - 本地Canvas边框绘制（4种风格）🆕
- `addWatermark()` - 水印添加
- 各类装饰元素绘制函数
...
3. 执行`database/schema.sql`初始化数据库
4. 验证数据库用户权限
